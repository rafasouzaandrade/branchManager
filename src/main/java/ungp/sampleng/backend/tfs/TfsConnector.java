...
public class TfsConnector {
	/**
	 * The URL to a TFS project collection, including virtual directory and collection name (like "http://server:8080/tfs/DefaultCollection").
	 */
	...
	public static String USERNAME = "rafael.andrade";
	public static String PASSWORD = "Teamore1607";
	...
	/**
	 * Set this to a writable TFS server folder (like "$/TeamProject/folder") where you can create and modify files. To run {@link BuildSample} and {@link GatedBuildSample}, this directory must
	 * contain a project that can be built using the files in the {@link #BUILD_CONFIG_FOLDER_PATH} folder path below.
	 */
	public static String MAPPING_SERVER_PATH = "";

	/**
	 * Set this to a writable local path (Windows or Unix style absolute path) that will be mapped to {@link #MAPPING_SERVER_PATH}. Used for {@link VersionControlSample}, {@link BuildSample}, and
	...
	/**
	 * Set this to a TFS server folder (like "$/TeamProject/TeamBuildTypes/MyTypes") where you have created upgrade-template compatible team build project files (TFSBuild.proj, TFSBuild.rsp) for
	 * building the project in {@link #MAPPING_SERVER_PATH}.
	 */
	...
	/**
	 * Connect to TFS using a set of credentials that uses the currently logged user in case no user name was provided otherwise it uses the provided credentials
	 * 
