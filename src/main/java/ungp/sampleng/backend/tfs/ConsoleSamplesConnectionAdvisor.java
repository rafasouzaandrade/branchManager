...
public class ConsoleSamplesConnectionAdvisor implements ConnectionAdvisor {
    ...
    private class SamplesHTTPClientFactory extends DefaultHTTPClientFactory {
        ...
        /**
         * Overrides configureClientProxy method to set the value of the http_proxy on the HTTP
         * Connection.
         */
        @Override
        public void configureClientProxy(HttpClient httpClient, HostConfiguration hostConfiguration, HttpState httpState, ConnectionInstanceData connectionInstanceData) {
            ...
            log.info("Proxy URL set to " + httpProxyURI.getHost() + ":" + (httpProxyURI.getPort() == -1 ? 80 : httpProxyURI.getPort()));

            hostConfiguration.setProxy(httpProxyURI.getHost(), httpProxyURI.getPort() == -1 ? 80 : httpProxyURI.getPort());
        }
    }
    ...
    static class SamplePersistenceStoreProvider extends DefaultPersistenceStoreProvider {
        ...
        public SamplePersistenceStoreProvider() {
            ...
            /*
             * Build a {@link PersistenceStore} which maps to a subdirectory inside the user's home
             * directory (more precisely, where the Java system property "user.home" points). See
             * the Javadoc on {@link VersionedVendorFilesystemPersistenceStore} for how the vendor
             * name, application name, and version are mixed into the path on each platform.
             */

            FilesystemPersistenceStore baseStore = new VersionedVendorFilesystemPersistenceStore("Microsoft-TEE-SDK-Sample-Vendor", "Connection Advisor Sample", "1.0");
    ...
    public WebServiceFactory getWebServiceFactory(ConnectionInstanceData instanceData) {
        return new DefaultWebServiceFactory(getLocale(instanceData), new DefaultTransportRequestHandler(instanceData, (ConfigurableHTTPClientFactory) getHTTPClientFactory(instanceData)));
    }
}
